...
class GetReferenceSwapRepository implements IGetReferenceSwapRepository{
  ...
  Future<ReferenceSwap> getReference({
    ...
    try{
      ...
      final getRepeatedFunction = FilterModesUtils.filterFunction[FilterModesUtils.onlyRepeated];
      final getNeedFunction = FilterModesUtils.filterFunction[FilterModesUtils.onlyMissing];


      final albumStickerRepeatedOtherUser = CreateSwapConfig
          .applyFliter(map[idOtherUser]!, getRepeatedFunction!);
      final albumStickerRepeatedSender = CreateSwapConfig
          .applyFliter(map[idSender]!, getRepeatedFunction);

      final albumStickerNeedOtherUser = CreateSwapConfig
          .applyFliter(map[idOtherUser]!, getNeedFunction!);
      final albumStickerNeedSender = CreateSwapConfig
          .applyFliter(map[idSender]!, getNeedFunction);


      return ReferenceSwap(
          stickersSender: _getStickerReferenceSwap(
              albumStickerNeedOtherUser,
              albumStickerRepeatedSender
          ),
          stickersNeed: _getStickerReferenceSwap(
              albumStickerNeedSender,
              albumStickerRepeatedOtherUser
          ),
      );
    }catch(e){
      rethrow;
  ...
  Album _getStickerReferenceSwap(
      Album needStickerUser,
      Album repeatedStickerOtherUser
  ){
    ...
    for(int i=0; i < 35; i++){
      ...
      if(needStickerUser.collectionStickers.containsKey(i) &&
        ...
        //Percorre as ficurinhas que o usuário precisa
        for(Sticker sticker in (needStickerUser.collectionStickers[i] as List<Sticker>)) {

          //Verifica se há figurinha correspondente a necessária no grupo
          //de repetidas do outro usuário, caso haja adicona.
          if(repeatedStickerOtherUser.collectionStickers[i]!.any((element) => element.text == sticker.text)) {
