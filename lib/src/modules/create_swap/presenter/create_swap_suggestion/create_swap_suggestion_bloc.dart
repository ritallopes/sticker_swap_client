...
class CreateSwapSuggestionBloc {
  ...
  Future<void> generateSuggestion() async {
    if (maxTradedStickers.text == null ||
        maxTradedStickers.text.isEmpty ||
        maxReceivedStickers.text == null ||
        maxReceivedStickers.text.isEmpty) {
  ...
  void _specialSwap() {
    ...
    if (referenceSwap.stickersNeed.collectionStickers.containsKey(0)) {
      List<Sticker> specialNeedStickers =
          referenceSwap.stickersNeed.collectionStickers[0]!;
    ...
    if (referenceSwap.stickersSender.collectionStickers.containsKey(0)) {
      List<Sticker> specialSendStickers =
          referenceSwap.stickersSender.collectionStickers[0]!;
  ...
  void _standardSwap() {
    ...
    for (int i = 1; i < 35; i++) {
      if (referenceSwap.stickersNeed.collectionStickers.containsKey(i)) {
        List<Sticker> stdNeedStickers =
            referenceSwap.stickersNeed.collectionStickers[i]!;
      ...
      if (referenceSwap.stickersSender.collectionStickers.containsKey(i)) {
        List<Sticker> stdSendStickers =
            referenceSwap.stickersSender.collectionStickers[i]!;
